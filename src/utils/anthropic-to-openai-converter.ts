...
export function convertNonStreamingResponse(
  ...
  const openAIResponse: OpenAIResponse = {
    ...
    usage: (() => {
      ...
      // Anthropic's input_tokens excludes cached tokens; OpenAI's
      // prompt_tokens is the full prompt, with cached_tokens as the subset
      const promptTokens =
        (u?.input_tokens || 0) +
...
function createUsageChunk(state: ConverterState): OpenAIStreamChunk | null {
  ...
  return {
    ...
    usage: (() => {
      ...
      console.log(
        `usage: prompt=${promptTokens} cached=${m.cache_read_input_tokens} ` +
          `cache_write=${m.cache_creation_input_tokens} output=${m.output_tokens} ` +
          `hit=${promptTokens ? Math.round((m.cache_read_input_tokens / promptTokens) * 100) : 0}%`,
      )
      return {
        prompt_tokens: promptTokens,
        completion_tokens: m.output_tokens,
        total_tokens: promptTokens + m.output_tokens,
        prompt_tokens_details: {
          cached_tokens: m.cache_read_input_tokens,
        },
      }
    })(),
  }
