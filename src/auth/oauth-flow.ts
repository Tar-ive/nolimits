...
if (!CLIENT_ID) {
  console.warn('ANTHROPIC_OAUTH_CLIENT_ID not set - OAuth features will be disabled')
}
...
export function getAuthorizationUrl(pkce: PKCE): string {
  if (!CLIENT_ID) {
    throw new Error('ANTHROPIC_OAUTH_CLIENT_ID environment variable is required for OAuth authentication')
  }
